...
districtSelect.addEventListener("change", () => {
  ...
  if (selectedDistrict && DATA[selectedDistrict]) {
    for (const zone in DATA[selectedDistrict].zones) {
      const option = document.createElement("option");
      option.value = zone;
...
function calcularTasacion(distrito, zona, area) {
  if (!DATA[distrito] || !DATA[distrito].zones[zona]) return 0;
  const precioM2 = DATA[distrito].zones[zona];
  let precio = precioM2 * area;
  return precio;
}
...
document.getElementById("calculate").addEventListener("click", function () {
  ...
  if (!distrito || !zona || isNaN(area) || area <= 0) {
    resultElement.innerText = "⚠️ Complete todos los campos.";
    rangeElement.innerText = "";
    return;
  }

  const precio = calcularTasacion(distrito, zona, area);
  const min = (precio * 0.9).toFixed(2);
  const max = (precio * 1.1).toFixed(2);

  resultElement.innerText = `💰 Tasación estimada: $${precio.toFixed(2)}`;
  rangeElement.innerText = `Rango de precio: entre $${min} y $${max}`;

  // 📊 Evento Analytics
  if (firebase.analytics) {
    firebase.analytics().logEvent("calcular_tasacion", {
      distrito: distrito,
      zona: zona,
      area: area,
      precio: precio
